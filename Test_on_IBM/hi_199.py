...
sample_ratio = 1.0  # 修改：保留100%节点（取消节点采样）
edge_sample_num = float('inf')  # 修改：保留所有边（取消边采样，inf表示不限制）
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('RGCN-XGBoost_HI_full_results.txt')
        ]
    )
...
setup_logging()
logging.info("=" * 60)
logging.info(f"HI数据集全量数据训练（采样比例{sample_ratio}）+ RGCN训练")
logging.info("=" * 60)
...
# ===================== 核心：全量数据处理（取消采样逻辑） =====================
def process_hi_dataset_light(accounts_path, trans_path, sample_ratio=1.0, edge_sample_num=float('inf')):
    """
    全量数据处理（取消节点/边采样）
    :param sample_ratio: 节点保留比例（固定为1.0）
    :param edge_sample_num: 每个节点最多保留的出边数（固定为inf，保留所有边）
    """
    # 1. 加载数据
    ...
    # 取消节点分层采样：直接使用所有账户
    logging.info(f"使用全量节点，无采样...")
    sampled_accounts = all_accounts  # 全部账户
    sampled_labels = account_labels  # 全部标签
    # 构建全量账户映射
    account2idx = {acc: idx for idx, acc in enumerate(sampled_accounts)}
    n_nodes = len(account2idx)
    logging.info(
        f"全量节点数: {n_nodes} | 洗钱账户数: {sum(sampled_labels)} | 正常账户数: {n_nodes - sum(sampled_labels)}")
    ...
    logging.info(f"节点特征维度: {node_features.shape}")

    # 6. 边处理（取消边采样，保留所有边）
    logging.info(f"加载全量边数据，无采样...")
    ...
    for _, row in trans_df.iterrows():
        ...
        if from_acc not in account2idx or to_acc not in account2idx or from_acc == to_acc:
            continue
        # 直接添加所有边，不采样
        edge_index.append([account2idx[from_acc], account2idx[to_acc]])
        edge_type.append(payment2type.get(fmt, 0))

    edge_index = torch.tensor(edge_index).T.long()
    edge_type = torch.tensor(edge_type).long()
    logging.info(f"全量边数: {edge_index.shape[1]} | 边类型数: {num_relations}")
...
if __name__ == "__main__":
    ...
    if device.type == "cpu":
        logging.warning("当前使用CPU训练，全量数据速度极慢，建议使用GPU并适当调大batch_size")
