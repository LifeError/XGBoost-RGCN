...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('Test(MLP)_logging_results.txt')  # 文件输出
        ]
    )
...
try:
    ...
    # 计算数据的类别不平衡程度
    logging.info(f'Class imbalance ratio: {np.mean(labels)}')
...
for epoch in range(num_epochs):
    ...
    # 输出当前batch的详细情况
    if VERBOSE:
        logging.info(f'Epoch: {epoch}, loss: {loss / num_batches}, time: {epoch_time}s')
